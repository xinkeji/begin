...
if ( ! class_exists( 'ZMOP_Options' ) ) {
  class ZMOP_Options extends ZMOP_Abstract {
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => '保存失败' ) );
      } else {
    ...
    public function add_admin_menu() {
      ...
      if ( $menu_type === 'submenu' ) {
        ...
        $menu_page = call_user_func( 'add_submenu_page', $menu_parent, esc_attr( $menu_title ), '<span class="bem"></span>' . esc_attr( $menu_title ), $menu_capability, $menu_slug, array( $this, 'add_options_html' ) );
      ...
      echo '<div class="zmop zmop-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="zmop-header-inner">';
          ...
          echo '<div class="zmop-header-right">';
            ...
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary zmop-top-save zmop-save'. esc_attr( $ajax_class ) .'" title="保存设置" value="保存设置" data-save="正在保存">';
            echo '</div>';
