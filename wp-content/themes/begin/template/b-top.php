...
<div class="cms-news-grid-container b-top">
	<?php
		$args = array(
			'post__in'  => explode( ',', be_get_option( 'blog_top_id' ) ),
			'orderby'   => 'post__in', 
			'order'     => 'DESC',
	...
	<?php if ( be_get_option( 'blog_top_n' ) < 4 ) { ?>
	...
	<?php endwhile; ?>
	<?php else : ?>
		<div class="be-none">首页设置 → 博客布局 → 其它模块 → 推荐文章，输入文章ID</div>
	<?php endif; ?>
	<?php wp_reset_postdata(); ?>
	<?php be_help( $text = '首页设置 → 博客布局 → 其它模块 → 推荐文章' ); ?>
	<div class="clear"></div>
</div>
<?php } ?>