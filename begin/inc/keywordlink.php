...
function wp_be_keywordlink_admininit() {
	add_management_page( "关键词设置","<span class='bem'></span>关键词", 'manage_options', 'keywordlink', 'wp_be_keywordlink_optionpage' );
}
...
echo '<h2 class="bezm-settings">关键词设置</h2>';
?>
...
<div class="wrap keyword-options">
	...
	<br />
	<hr />
	<form name=wp_be_keywordlink_deactivate id=wp_be_keywordlink_deactivate method=POST action=''>
		<p></p>
...
function wp_be_keywordlink_cvsimport() {
	...
	update_option(WP_KEYWORDLINK_OPTION,$links);
	wp_be_keywordlink_topbarmessage("导入完成，替换了 $replace 关键词, 忽略 $skip 个, 添加 $added 个关键词");
	}
	else 
