...
function be_save_image( $post_id, $post ) {
	global $wpdb;
	if ( $post->post_status == 'publish' ) {
		...
		if ($num) {
			...
			$bes_site = $_SERVER['HTTP_HOST'];
			foreach ( $matches[1] as $src ) {
				if ( isset( $src ) && strpos( $src, $bes_site ) === false ) {
					...
					if ( file_exists( $file_path ) && filesize( $file_path ) > 0 ) {
						...
						if ( pathinfo( $file_path, PATHINFO_EXTENSION ) == 'tmp' ) {
							$file_path = bes_handle_ext( $file_path, $arr[1], $wp_upload_dir['path'], $file_name, 'tmp' );
						} 
						//elseif ( pathinfo($file_path, PATHINFO_EXTENSION) == 'webp' ) {
							//$file_path = bes_handle_ext( $file_path, $arr[1], $wp_upload_dir['path'], $file_name, 'webp' );
						//}
						$post->post_content  = str_replace( $src, $wp_upload_dir['url'] . '/' . basename( $file_path ), $post->post_content );
						$attachment = bes_get_attachment_post( basename( $file_path ), $wp_upload_dir['url'] . '/' . basename( $file_path ) );
...
function bes_handle_ext( $file, $type, $file_dir, $file_name, $ext ) {
	switch ( $ext ) {
		case 'tmp':
			if ( rename( $file, str_replace( 'tmp', $type, $file ) ) ) {
				if ( 'webp' == $type ) {
					return bes_image_convert( 'webp', 'jpeg', $file_dir . '/' . str_replace( 'tmp', $type, $file_name ) );
				}
				return $file_dir . '/' . str_replace('tmp', $type, $file_name );
			}
		case 'webp':
			if ( 'webp' == $type ) {
				return bes_image_convert( 'webp', 'jpeg', $file );
			} else {
...
function bes_image_convert( $from='webp', $to='jpeg', $image ) {
...
function bes_get_attachment_post( $filename, $url ) {
...
add_action( 'save_post', 'be_save_image', 120, 2 );