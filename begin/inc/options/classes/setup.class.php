...
if ( ! class_exists( 'ZMOP_Setup' ) ) {
  class ZMOP_Setup {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'ZMOP', 'setup' ) );
      add_action( 'init', array( 'ZMOP', 'setup' ) );
      add_action( 'switch_theme', array( 'ZMOP', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'ZMOP', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'ZMOP', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'ZMOP', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'ZMOP', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'ZMOP_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( ZMOP_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ZMOP_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ZMOP_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'ZMOP_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              ZMOP_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'zmop_override', 'zmop-override' );
    ...
    public static function includes() {
      ...
      self::include_plugin_file( 'functions.php'  );
      ...
      self::include_plugin_file( 'classes/admin-options.class.php' );
        ...
        self::include_plugin_file( 'fields.php'   );
      // Include all framework fields
      $fields = apply_filters( 'zmop_fields', array(

      ) );

      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'ZMOP_Field_'. $field ) && class_exists( 'ZMOP_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_zmop-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'zmop_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      // Main scripts
      wp_enqueue_script( 'zmop-plugins', self::include_plugin_url( 'assets/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'zmop', self::include_plugin_url( 'assets/main'. $min .'.js' ), array( 'zmop-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'zmop', 'zmop_vars', array(
        'color_palette'     => apply_filters( 'zmop_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => '单击「确定」进行下一步操作，该操作可能会丢失部分配置！',
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'zmop' ),
          'searching_text'  => esc_html__( 'Searching...', 'zmop' ),
          'no_results_text' => esc_html__( 'No results found.', 'zmop' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'ZMOP_Field_' . $field['type'];
    ...
    public static function add_admin_body_class( $classes ) {
      ...
      if ( apply_filters( 'zmop_fa4', false ) ) {
        $classes .= 'zmop-fa5-shims';
      }
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'zmop' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' zmop-pseudo-field' : '';
      ...
      echo '<div class="zmop-field zmop-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="zmop-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="zmop-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) ) ? '<div class="zmop-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'ZMOP_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'zmop' ) .'</p>';
        }
...
ZMOP_Setup::init( __FILE__, false );
 ...
 *
 */
if ( ! class_exists( 'ZMOP' ) ) {
  class ZMOP extends ZMOP_Setup{}
}
